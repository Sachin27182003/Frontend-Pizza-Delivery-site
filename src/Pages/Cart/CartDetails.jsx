...
import { addProductToCart, getAllProductFromCarts, removeProductFromCart } from "../../Redux/Slices/CartSlice";
...
function CartDetails(){
   ...
   async function fetchCartDetails() {
    const details = await dispatch(getAllProductFromCarts());
    setCartDetails(details?.payload?.data?.items);
    totalQuantity()
   }
                  ...
                  {cartDetails?.map((item) => (
                    <div key={item._id} className="p-4 text-gray-900 rounded-lg shadow-sm bg-gradient-to-r from-amber-50 to-orange-300 md:p-6 border">
                      <div className="space-y-4 md:flex md:items-center md:justify-between md:gap-6 md:space-y-0">
                        <img
                          className="hidden w-20 h-20 object-contain dark:block rounded-md"
                        ...
                        <div className="flex-1 w-full min-w-0 md:order-2 md:max-w-md">
                          <p className="text-base font-medium text-gray-900 hover:underline">
                            <Link to={`/product/${item?._id}`}>
                              {`${item?.product?.name}, ${item?.product?.description}, Category: ${item?.product?.category}`}
                            </Link>
                          </p>
                          <p> ₹{item?.product?.price} </p>
                          <p> Quantity: {item?.quantity} </p>
                          <button className="text-green-500" onClick={() =>handleCart(item?.product?._id)}>➕ Add +1</button>
                  ...
                  <div className="space-y-4">
                    ...
                    <dl className="flex items-center justify-between gap-4 pt-2 border-t border-gray-200 dark:border-gray-700">
                      ...
                      <dd className="text-base font-bold ">
                        ...
                        {cartDetails?.length === 0
                          ? ''
                          : cartDetails?.reduce((acc, item) => acc + item?.quantity*item?.product?.price , 0) }
                      </dd>
                    </dl>
                  </div>
                  {cartDetails?.length > 0 && (
